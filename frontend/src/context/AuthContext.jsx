...
export function AuthProvider({ children }) {
  ...
  const loginWithTokens = useCallback(async (accessToken, refreshToken) => {
    ...
    localStorage.setItem("myjob_refresh_token", refreshToken);
    try {
  ...
  const loginWithMicrosoft = async () => {
    try {
      ...
      throw new Error(err.response?.data?.detail || "Microsoft SSO not available");
    }
  };
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, register, logout, refreshUser, loginWithMicrosoft, loginWithTokens }}>
      {children}
