...
import { LayoutDashboard, Briefcase, BarChart2, MessageSquare, LogOut, Moon, Sun, ChevronLeft, ChevronRight, Bell, FileText, Bot, Shield, Activity } from "lucide-react";
...
const NAV = [
  ...
  { path: "/admin", icon: Activity, label: "Monitoring" },
        ...
        <div className={`flex items-center ${collapsed ? "justify-center px-3" : "px-5"} h-[56px] border-b border-border`}>
          ...
          {!collapsed && (
            <div className="flex items-center gap-1.5 ml-2">
              <span className="font-['Plus_Jakarta_Sans'] text-[16px] font-bold tracking-tight text-foreground">
                My<span style={{ color: "rgb(var(--primary))" }}>Job</span>
              </span>
              <span className="px-1 py-0.5 rounded text-[7px] font-['JetBrains_Mono'] font-bold" style={{ background: "rgba(var(--primary), 0.1)", color: "rgb(var(--primary))" }}>v2</span>
            </div>
          )}
        </div>
              ...
              <div className="px-2.5 py-2.5 rounded-xl bg-muted">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className="text-xs font-['Plus_Jakarta_Sans'] font-semibold text-foreground truncate max-w-[90px]">{user.name}</span>
                    {user.auth_provider === "microsoft" && (
                      <span className="flex-shrink-0 w-3.5 h-3.5 rounded flex items-center justify-center" style={{ background: "#0078D4" }}>
                        <svg width="7" height="7" viewBox="0 0 21 21"><rect x="1" y="1" width="9" height="9" fill="#F25022"/><rect x="11" y="1" width="9" height="9" fill="#7FBA00"/><rect x="1" y="11" width="9" height="9" fill="#00A4EF"/><rect x="11" y="11" width="9" height="9" fill="#FFB900"/></svg>
                      </span>
                    )}
                  </div>
